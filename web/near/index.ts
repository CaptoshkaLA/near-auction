...
// NEW
export const createAuction = async (lotName: string, lotImageUrl: string) => {
  const gas = 30_000_000_000_000;
  // @ts-ignore
  return contract?.create_auction({ lotName, lotImageUrl }, gas);
};
...
export const cancelAuction = async (auctionId: number) => {
  const gas = 30_000_000_000_000;
  // @ts-ignore
  return contract?.cancel_auction({ auctionId }, gas);
};
...
export const endAuction = async (auctionId: number) => {
  const gas = 30_000_000_000_000;
  // @ts-ignore
  return contract?.end_auction({ auctionId }, gas);
};

//NEW
const pickle = require('node-pickle');

export const predictCar = async (carData: any) => {
  const modelPath = '../car_linear.pkl'; // Путь к файлу модели
  const model = await loadModel(modelPath);
  const prediction = model.predict([carData]);
...
const loadModel = async (modelPath: string) => {
  ...
  const model = pickle.loads(new Uint8Array(modelBuffer));
  return model;
};
